...
class RegistryState:
    def __init__(self) -> None:
        ...
        self.knowledge_hash_index: dict[str, str] = {}
                ...
                for item in data.get("knowledgePacks", []) or []:
                    ...
                    if pack.pack_id:
                        ...
                        if pack.content_hash:
                            self.knowledge_hash_index[pack.content_hash] = pack.pack_id
        except Exception:
    ...
    async def get_knowledge_by_hash(self, content_hash: str) -> KnowledgePack | None:
        async with self.lock:
            pack_id = self.knowledge_hash_index.get(content_hash)
            if not pack_id:
                return None
    ...
    async def touch_knowledge(self, pack_id: str) -> None:
        async with self.lock:
            pack = self.knowledge_packs.get(pack_id)
            if not pack:
                return
            pack.uses += 1
            pack.updated_ts = _utc_ts()

    async def rate_knowledge(self, pack_id: str, delta: int) -> KnowledgePack | None:
        async with self.lock:
            pack = self.knowledge_packs.get(pack_id)
            if not pack:
                return None
    ...
    async def list_knowledge(
        ...
        *,
        ...
        if not include_pending:
            packs = [p for p in packs if p.status == "approved"]
        if kind:
        ...
        packs.sort(key=lambda p: (p.score, p.updated_ts), reverse=True)
