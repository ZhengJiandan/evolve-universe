...
class RegistryServer:
    ...
    @staticmethod
    def _quality_score(*, name: str, summary: str, tags: list[str], content: str) -> int:
        """Compute a simple quality score for knowledge packs (0-100)."""
        score = 0
        name_len = len(name.strip())
        summary_len = len(summary.strip())
        content_len = len(content.strip())

        # Content length contributes most (up to 40 points).
        score += min(40, int(content_len / 25))

        # Summary length (up to 20 points).
        score += min(20, int(summary_len / 10))

        # Tags (up to 10 points).
        score += min(10, len(tags) * 2)

        # Name length (up to 10 points).
        if name_len:
            score += min(10, max(2, int(name_len / 8)))

        # Structure signals (up to 20 points).
        structure_score = 0
        if "## " in content:
            structure_score += 10
    ...
    @staticmethod
    def _compute_score(*, quality_score: int, uses: int, upvotes: int, downvotes: int) -> float:
        base = float(quality_score) / 10.0
        popularity = math.log1p(max(0, int(uses)))
        rating = (int(upvotes) * 2.0) - (int(downvotes) * 3.0)
        return round(base + popularity + rating, 3)
    ...
    async def _handler(self, ws: WebSocketServerProtocol) -> None:
        ...
        try:
            ...
            if env.type == "register":
                ...
                # Treat non-register initial messages as normal requests (no separate handshake).
    ...
    async def _handle(self, env: Envelope, ws: WebSocketServerProtocol) -> None:
        ...
        if env.type == "knowledge_publish":
            ...
            # Dedup by content hash (strict).
            dup = await self.state.get_knowledge_by_hash(content_hash)
            if dup and dup.pack_id != pack_id:
                await ws.send(
                    make_envelope(
                        "error",
                        id=env.id,
                        payload={"message": f"duplicate content (pack_id={dup.pack_id})"},
                    ).to_json()
                )
                return
            ...
            score = self._compute_score(
                ...
                upvotes=existing.upvotes if existing else 0,
                downvotes=existing.downvotes if existing else 0,
            )
        ...
        if env.type == "knowledge_list":
            ...
            include_pending = bool(payload.get("includePending", False)) and self._check_token(token)
            packs = await self.state.list_knowledge(
                kind=kind, tag=tag, owner=owner, limit=limit, include_pending=include_pending
            )
            await ws.send(
                make_envelope(
        ...
        if env.type == "knowledge_get":
            ...
            if not pack:
                ...
                return
            await self.state.touch_knowledge(str(pack_id))
            pack = await self.state.get_knowledge(str(pack_id))
            if pack:
                pack.score = self._compute_score(
                    quality_score=pack.quality_score,
                    uses=pack.uses,
                    upvotes=pack.upvotes,
                    downvotes=pack.downvotes,
                )
                if pack.status != "approved" and pack.score >= 10:
        ...
        if env.type == "knowledge_rate":
            token = (env.payload or {}).get("registryToken", "")
            if self.cfg.registry_token and not self._check_token(token):
                await ws.send(make_envelope("error", id=env.id, payload={"message": "invalid registry token"}).to_json())
                return
            pack_id = (env.payload or {}).get("id")
            delta = int((env.payload or {}).get("delta", 0) or 0)
            if not pack_id or delta == 0:
                await ws.send(make_envelope("error", id=env.id, payload={"message": "missing id/delta"}).to_json())
                return
            pack = await self.state.rate_knowledge(str(pack_id), delta)
            if not pack:
                await ws.send(make_envelope("error", id=env.id, payload={"message": "pack not found"}).to_json())
                return
            pack.score = self._compute_score(
